...
def ID_discriminator_model_loader(weights_path=None, hack=False,normalise=True):
    ...
    model = senet50_256(weights_path,hack, normalise)

    return model

def senet50_256(weights_path=None, hack =False, normalise=True):
    
    model = Senet50_256(normalise)
...
class Senet50_256(nn.Module):
    ...
    def forward(self, data):
        ...
        feat_extract_norm = F.normalize(feat_extract, p=2, dim=1)

        if self.normalise:
            return feat_extract_norm
        else:
            return feat_extract
